...
export async function GET(req) {
  ...
  const filterValid = searchParams.get('realOnly') === 'true'; // [신규] 60초 이상 유저만 볼지 여부
  ...
  // [기능 핵심] 실사용자 데이터와 테스트 데이터 분리 (60초 기준)
  ...
  // 2. Fetch Responses for Question-level Timing (최근 5000건)
  // [강화] 응답 데이터도 실제 사용자 세션(60초 이상) 건만 필터링하도록 세션 ID 추출
  ...
  const dailyList = Object.values(dailyStats).map(d => ({
    ...d, avgTime: (d.timeSum / d.count / 1000).toFixed(1)
