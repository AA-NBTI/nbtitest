  ...
  try {
    ...
    if (targetSessionIds.length > 0) {
      const { data: responses } = await supabase
        ...
        .in('session_id', targetSessionIds.slice(0, 100))
        .limit(5000);
      safeResp = responses || [];
    }

    // [중간 생략] 문항 정보 및 그룹화 로직 유지
    ...
    Object.entries(qStats).forEach(([id, s]) => {
      ...
      if (axisGroup) { axisGroup.items.push(item); axisGroup.total += s.count; }
      const tGroup = versionGrouped[info.testType] || versionGrouped.basic;
      tGroup.items.push(item); tGroup.total += s.count;
    });
    ...
    return Response.json({
      ...
      metrics: {
        ...
        avgDurationSec: targetResults.length > 0 ? (targetResults.reduce((acc, curr) => acc + (curr.total_time_ms || 0), 0) / (targetResults.length || 1) / 1000).toFixed(1) : 0
      },
      types: {
      ...
      hourly: hourlyDistribution, // [신규] 시간대별 데이터
      groupedQuestions, 
      versionGrouped
    });
  } catch (err) {
