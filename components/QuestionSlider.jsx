...
export function QuestionSlider() {
  ...
  const handleSelect = (val) => {
    ...
    setAnswer(question.question_id, question.version_id, val, responseTimeMs, question.axis, question.polarity, question.isAd);
  ...
  // 고정 배너 렌더러 (모바일 스티키 최적화)
  const renderFixedBanner = (ad, isStickyBottom = false) => {
    if (!ad) return null;
    return (
      <a 
        href={ad.link_url} 
        ...
        onClick={() => recordAdClick(ad.ad_id)}
        className={isStickyBottom 
  ...
  return (
    <div className="w-full h-full flex flex-col items-center gap-4 pb-[60px]">
      {/* [v16] 테스트 중 상단 광고 */}
      <div className="w-full max-w-lg mb-2">
         {renderFixedBanner(fixedAds.top)}
      </div>

      <div className="w-full max-w-lg mx-auto overflow-hidden px-4 py-8 bg-white rounded-3xl shadow-sm border border-slate-100">
              ...
              >
                ...
                {question.attachedAd.banner_img_url && (
                  <div className="w-full aspect-[2/1] mb-3 bg-slate-100 rounded-lg overflow-hidden border border-slate-200 relative flex items-center justify-center shadow-inner">
                     ...
                     <img src={question.attachedAd.banner_img_url} className="w-full h-full object-cover group-hover:scale-[1.03] transition-transform duration-500" alt="스폰서 미디어 배너" />
                  </div>
                )}
        ...
        <div className="w-full relative mb-6">
          ...
          <div className="flex justify-between items-center w-full">
            {LIKERT_OPTIONS.map((opt) => (
              <button key={opt.value} onClick={() => handleSelect(opt.value)} disabled={isTransitioning} className={`group relative flex flex-col items-center justify-center transition-all duration-200 ${opt.size} rounded-full border-[3px] ${isTransitioning ? 'cursor-default' : 'cursor-pointer hover:scale-110'} ${selectedValue === opt.value ? `${opt.color} border-transparent` : 'bg-white border-gray-200'} active:scale-95`} />
            ))}
          </div>
        </div>
      ...
      {/* [v16] 모바일 최적화 하단 고정 스티키 배너 (320x50 표준) */}
      {renderFixedBanner(fixedAds.bottom, true)}
    </div>
  );
}
