...
export function AdSlot({ slotId, mbtiType = 'all' }) {
  ...
  useEffect(() => {
    async function fetchAd() {
      try {
        ...
        // API가 이미 is_active=true + placement 필터 후 반환 → 추가 필터는 MBTI 타겟 매칭만
  ...
  // MAIN_TOP: 375×50 모바일 업계 표준 상단 고정 스티키 배너 (배경 없이 정확한 50px만 노출)
  if (slotId === 'MAIN_TOP') {
    return (
      <a
        href={ad.link_url}
        ...
        className="flex w-full items-center justify-center overflow-hidden bg-white relative group"
  ...
  // 기본 광고 슬롯 렌더링 (RESULT_TOP, RESULT_BOTTOM 등)
  const isResultSlot = slotId.includes('RESULT');
  const containerAspect = isResultSlot ? 'aspect-[3.2/1]' : 'aspect-[2/1]';

  return (
    <div className={`w-full ${isResultSlot ? 'max-w-[320px] rounded-2xl' : 'max-w-[400px] mt-8 mb-4 rounded-3xl'} overflow-hidden border border-slate-200 bg-white shadow-sm hover:shadow-md transition-all group`}>
      <a 
        href={ad.link_url} 
        target="_blank" 
        rel="noopener noreferrer"
